...
for subj in args.subjects:
    ...
    for task in tasks:
        ...
        base = f"{sub_dir}/fsl_stats/sub-{subj}_task-{task}_contrasts.feat"
        ica_path = f"{base}/filtered_func_data.ica"

        func_file = f"{base}/filtered_func_data.nii.gz"
        melodic_ic_file = f"{ica_path}/melodic_IC.nii.gz"
        melodic_mix_file = f"{ica_path}/melodic_mix"
        zstat_file = f"{base}/stats/zstat1.nii.gz"
        ...
        subject_maps.to_filename(f"{base}/sub-{subj}_{task}_dual_regression_maps.nii.gz")
