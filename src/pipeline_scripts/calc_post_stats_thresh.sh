...
# Updated to include ICA map transformations and statistics, May 2025
# Updated to remove TFCE thresholding and use unthresholded TFCE map (values > 0) for activated voxels, May 2025
# Updated to mask TFCE activated voxels with t-stat map, May 2025
# Updated to process only MNI space and split t-map for left/right hemispheres, May 2025
...
calculate_dice() {
    local t_map=$1    # TFCE map (unthresholded, values > 0) or other map
    local z_map=$2    # Thresholded Z-map
    local overlap=$(fslstats "$t_map" -k "$z_map" -l 0 -V | awk '{print $1}')
    local total_t=$(fslstats "$t_map" -l 0 -V | awk '{print $1}')
...
# Function to preprocess subject (skull-strip T1w and resample ROIs in MNI space)
preprocess_subject() {
    local subject=$1
...
process_post_stats() {
    ...
    # Task-specific ROI mappings for MNI space
